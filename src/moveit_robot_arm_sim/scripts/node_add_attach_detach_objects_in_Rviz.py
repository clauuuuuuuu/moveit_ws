...
class MyRobot:
    ...
    def add_cylinder(self, name, x, y, z, radius, height, timeout=4):
        """
        ...
        # Create a pose message for the cylinder
        ...
        cylinder_pose.pose.orientation.z = 0
        ...
        # Check if the cylinder is added successfully
...
def main():
    ...
    while not rospy.is_shutdown():
        task = input("Enter add to add object (or add1 add2 add3), attach to attach object, detach to detach object and remove to remove object: ")
        
        if task == "add":
            # Using a cylinder with radius 0.15 and height 0.08 (for a long cylinder)
            response = robotArm.add_cylinder("package$1", -0.91, 0.36, 0.015, 0.15, 0.08)
            
            if response == True:
                rospy.loginfo('\033[32m' + "Added Cylinder in Rviz: {}".format(robotArm.box_name) + '\033[0m')

        if task == "add1":
            response = robotArm.add_cylinder("package$1", 0.18, -0.25, 0.015, 0.15, 0.08)
    
            if response == True:
                rospy.loginfo('\033[32m' + "Added Cylinder in Rviz: {}".format(robotArm.box_name) + '\033[0m')

        if task == "add2":
            response = robotArm.add_cylinder("package$1", 0.35, -0.26, 0.015, 0.15, 0.08)
